...
import Waitlist from './Waitlist'; // Importamos el componente Waitlist para usarlo en la pestaña
...
export default function GenesisMint() {
  const [view, setView] = useState<'mint' | 'waitlist'>('mint');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-purple-900 to-black p-8">
          ...
          <Card className="bg-gradient-to-r from-gray-900 to-black border-2 border-yellow-500 p-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-yellow-400">{userBerries} $Berries</p>
              </div>
            </div>
          </Card>
        </div>
      ...
      {/* Navegación entre Mint y Waitlist */}
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex justify-center space-x-4">
          <button 
            className={`px-6 py-3 font-bold rounded-lg transition-all ${view === 'mint' ? 'bg-yellow-600 text-black shadow-lg shadow-yellow-500/50' : 'bg-gray-700 text-white hover:bg-gray-600'}`}
            onClick={() => setView('mint')}
          >
            Minting de Casetes
          </button>
          <button 
            className={`px-6 py-3 font-bold rounded-lg transition-all ${view === 'waitlist' ? 'bg-yellow-600 text-black shadow-lg shadow-yellow-500/50' : 'bg-gray-700 text-white hover:bg-gray-600'}`}
            onClick={() => setView('waitlist')}
          >
            Waitlist (Reclutamiento Nakama)
          </button>
        </div>
      </div>
